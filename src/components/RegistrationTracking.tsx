...
const getMockEvaluationsForPhase = (phaseName: string): any[] => {
  if (phaseName.includes('Mérito Cultural') || phaseName.includes('Avaliação de Mérito')) {
    return [
      {
        reviewerId: 'rev1',
        ...
        score: 40,
        totalPossible: 60,
        comments: "A proposta de publicação do livro VERSOS E POESIAS DOM JOSÉ LUIZ AZCONA é um livro de memória que mostra os 37 anos de episcopado de Dom Azcona, com o objetivo de valorizar a história e o legado deixado pelo Bispo Emérito do Marajó, que lutou incansavelmente contra o tráfico humano e a exploração de crianças. O livro será ilustrado e terá distribuição gratuita, apresentando excelente adequação cultural.",
        criteriaScores: [
      ...
      },
      {
        ...
        score: 40,
        totalPossible: 60,
        comments: "Identificação: Vanderlei Lobato de Castro. O projeto possui relevância para a esfera educacional e cultural paraense, ao resgatar as memórias de Dom Azcona no Marajó. O proponente demonstra ampla inserção e comprometimento com o setor literário local.",
        criteriaScores: [
  ...
  if (phaseName.includes('Habilitação - Avaliação')) {
    return [
      {
        reviewerId: 'rev-hab-eval',
        reviewerName: "Parecerista Técnico Principal",
        score: 100,
        totalPossible: 100,
        comments: "Análise técnica realizada e aprovada. Toda a documentação enviada e os critérios de habilitação foram integralmente atendidos. Recomenda-se a habilitação definitiva e encaminhamento para recursos financeiros.",
        criteriaScores: [
          { label: 'Conformidade Jurídica', score: 25, maxScore: 25 },
          { label: 'Adequação aos Requisitos de Participação', score: 25, maxScore: 25 },
          { label: 'Pertencimento Territorial', score: 25, maxScore: 25 },
          { label: 'Pontuação de Bonificação', score: 25, maxScore: 25 }
        ]
      }
    ];
  }

  if (phaseName.includes('Habilitação - Anexos')) {
    return [
      {
        reviewerId: 'rev-hab-doc',
        reviewerName: "Analista de Documentação",
        score: 50,
        totalPossible: 50,
        comments: "Todos os anexos obrigatórios exigidos no edital foram enviados e estão legíveis, válidos e devidamente assinados. Enquadramento positivo.",
        criteriaScores: [
          { label: 'Regularidade de Certidões', score: 10, maxScore: 10 },
          { label: 'Documento de Identificação', score: 10, maxScore: 10 },
          { label: 'Portfólio Consolidado', score: 10, maxScore: 10 },
          { label: 'Comprovante de Residência', score: 10, maxScore: 10 },
          { label: 'Declarações Assinadas', score: 10, maxScore: 10 }
        ]
      }
    ];
  }

  if (phaseName.includes('inscrições')) {
    return [
      {
        reviewerId: 'rev-inscr',
        reviewerName: "Sistema / Triagem de Inscrição",
        score: 10,
        totalPossible: 10,
        comments: "Inscrição recebida com sucesso e enquadrada preliminarmente na categoria selecionada. Chave de segurança gerada.",
        criteriaScores: [
          { label: 'Submissão Eletrônica', score: 5, maxScore: 5 },
          { label: 'Recibo de Entrega', score: 5, maxScore: 5 }
        ]
      }
    ];
  }

  if (phaseName.includes('Resultado') || phaseName.includes('resultado')) {
    return [
      {
        reviewerId: 'rev-res',
        reviewerName: "Comissão Deliberativa Final",
        score: 100,
        totalPossible: 100,
        comments: "Consolidado final de recursos homologado pela secretaria. Homologação final deferida e publicada.",
        criteriaScores: [
          { label: 'Homologação e Publicação', score: 50, maxScore: 50 },
          { label: 'Julgamento de Recursos', score: 50, maxScore: 50 }
        ]
      }
    ];
  }
  ...
  return [
    {
      reviewerId: 'rev-gen',
      reviewerName: "Avaliador Geral",
      score: 85,
      totalPossible: 100,
      comments: "Fase analisada com parecer favorável. O projeto atende os requisitos estabelecidos.",
      criteriaScores: [
        { label: 'Critério Geral do Edital', score: 85, maxScore: 100 }
      ]
    }
  ];
};
